# 기능 소유권과 `pi-cmux` 대비 현황

이 문서는 `pi-cmux-presence`, `pi-subagent` producer 예시, cmux 공식 Pi hook의 책임을 한곳에 정리한다. `pi-cmux-presence`는 외부 producer를 import하지 않으며 같은 Pi process의 [versioned event 계약](event-contract.md)만 소비한다. 구체적인 producer 경계는 [`pi-subagent` 연동 문서](pi-subagent-integration.md)를 함께 참고한다.
...
| 알림·flash·log | capability/config에 따라 best-effort 출력 | 필요한 경우 generic attention을 발행 가능 | replay attention은 `none` 권장 |
...
- 공통 채널은 `pi-presence:update:v1`과 `pi-presence:ready:v1`이다.
- wire DTO는 두 저장소에 의도적으로 중복 선언한다. 런타임 package dependency나 shared lifecycle 모듈을 만들지 않는다.
- `ready.consumer`는 UI capability 힌트이며 인증·명령 채널이 아니다.
- `pi-cmux-presence`는 source가 subagent인지 테스트 runner인지에 관계없이 generic update를 검증한다. 다만 `pi`와 `pi-todo`는 local source로 예약한다.
- `pi-subagent` 같은 producer는 필요하면 `source.id: "pi-subagent"`처럼 안정된 비예약 ID를 사용할 수 있다. consumer의 수락·부재가 producer의 실행·결과·취소·cleanup authority를 바꾸어서는 안 된다.
- event listener, 잘못된 host session ID와 cmux socket 실패는 lifecycle 결과를 바꾸지 않는다.
- generic producer label은 형식 검증과 byte-safe 축약만 거치므로 producer가 민감정보와 신뢰할 수 없는 원문을 제외한다.

## `pi-cmux` 대비

| `pi-cmux` 기능군 | 현재 대체 여부 | 결정 |
| --- | --- | --- |
| Pi status, token/cost, completion attention | 대체 | presence의 socket 경로 사용 |
...
`pi-cmux`의 command/workflow가 필요하면 해당 package를 선택적으로 함께 둘 수 있다. 이 경우 status/sidebar/notification 기능은 한쪽만 활성화해 중복을 피한다.
