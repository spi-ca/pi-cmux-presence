...
| `PI_CMUX_PRESENCE_TIMEOUT_MS` | `1000` | 정수 100–30000, 개별 요청의 연결 전·후 소켓 검증, 연결·응답, startup 소켓 경로 검증과 공식 hook probe 제한 |
...
identity와 소켓 선택은 fail-closed입니다. 검증에 실패하면 cmux 쓰기를 만들지 않습니다. 공식 hook probe가 timeout·abort·오류·크기/형식 제한에 걸리면 native integration authority도 fail-closed로 억제하며, 실제 미해결 probe 하나가 settle할 때까지 반복 start는 새 probe를 만들지 않습니다. 연결 전/후 fingerprint 또는 startup 경로 검증이 timeout·abort되면 대기 중인 filesystem 작업을 기다리지 않고 해당 요청·lifecycle 전이를 끝내며 늦은 결과를 사용하지 않습니다. post-connect fingerprint가 **미해결인 동안** request write 전의 data/end/close/error는 response가 되지 않고 요청을 거부·fail-close합니다. 미해결 transport fingerprint lease는 runtime 공유 gate에서 실제 settle까지 하나만 남으므로 replacement transport도 그동안 새 validation을 시작할 수 없습니다. 미해결 startup resolver는 settle 전까지 다음 epoch의 검증도 막습니다. 반대로 연결 전 connect error/timeout 및 두 fingerprint가 완료된 뒤의 정상 응답 대기는 현재 요청만 실패시키며 transport를 close하지 않아 다음 요청이 복구될 수 있습니다. 연결·시간 초과·큐 포화·응답 검증 실패는 best-effort 관찰 쓰기만 유실시키며 Pi 작업을 실패시키지 않습니다. 응답 한 줄을 받기 전 소켓이 정상 종료되면 요청은 시간 초과를 기다리지 않고 실패합니다. 직렬 queue에서 아직 실행되지 않은 같은 key의 UI 쓰기는 latest-write-wins로 병합되어 같은 promise를 공유합니다. 실행 중인 요청과 서로 다른 key는 교체하지 않지만, key 없는 낮은 우선순위 feed는 FIFO로 두고 primary 출력보다 뒤에 배치하거나 포화 시 교체합니다. 분리된 세션의 정리는 clear·lifecycle·resume·close 작업을 순차 best-effort로 시도하므로 responsive한 동안 각 작업에 기회가 있습니다. 단, 전체 정리에는 내부 bounded deadline(현재 `min(5초, max(750ms, 요청 timeout × 4))`)이 적용되며, 만료되면 활성 작업을 중단하고 남은 작업은 건너뜁니다. session start hook은 이 작업을 기다리지 않지만 정상적인 `session_shutdown` hook은 bounded cleanup promise를 반환하고 await하며, observer 오류는 삼켜 Pi 작업 실패로 전파하지 않습니다. 따라서 cmux를 사용할 수 없거나 deadline이 만료되면 어느 정리 요청의 전달도 보장하지 않습니다.

정상 shutdown을 await해도 process crash나 `SIGKILL`처럼 hook이 실행될 수 없는 종료는 복구하지 못합니다. 현재 process 간 stale status를 찾아 지우는 persisted reconciliation은 없습니다. lease/TTL 만료 또는 owner identity 기반 회수는 향후 recovery 설계 선택지이며 현재 구현된 동작이나 설정이 아닙니다.
