...
| `PI_CMUX_PRESENCE_TIMEOUT_MS` | `1000` | 정수 100–30000, 개별 요청의 연결 전·후 소켓 검증, 연결·응답 및 startup 소켓 경로 검증 제한 |
